  ...
  db.find({id: uid}, function(err, docs) {
    ...
    db.find({}, function(err, docs) {
      ...
      for (let k=0; k<docs.length; k++){
        for (let i=0; i<docs[k].length; i++){
          let person = {};
          let pname = docs[k][i].name;
          let pid = docs[k][i].id;
          person[pname] = [];
          for(let j=0; j<(docs[k][i].places.data ? docs[k][i].places.data.length : 0); j++){
            if (docs[k][i].places.data[j].place.location && docs[k][i].places.data[j].place.location.city) {
