...
def auto():
    if request.method == 'POST': 
        ...
        if not request.json['fromCity']:
            rsp = "City can't be blank. Please try again."
            return render_template('index.html',rsp=rsp)
        else:
            baseurl = 'https://api.sandbox.amadeus.com/v1.2/airports/autocomplete?apikey='
            r = requests.get(baseurl+secret+'&term='+str(request.json['fromCity']))
...
def location():
    if request.method == 'POST':
        ...
        if not request.json['fromCity']:
            ...
            baseurl = 'http://api.sandbox.amadeus.com/v1.2/location/'+str(request.json['fromCity'])+'/?apikey='
	    print(baseurl)
...
def topten(): 
    if request.method == 'POST':  
        ...
        if not request.json['period'] or not request.json['fromAirp']: 
            rsp = "airport and period  can't be blank. Please try again." 
            return render_template('index.html',rsp=rsp) 
        else: 
            baseurl = 'https://api.sandbox.amadeus.com/v1.2/travel-intelligence/top-destinations?apikey=' 
            r = requests.get(baseurl+secret+'&period='+str(request.json['period'])+'&origin='+str(request.json['fromAirp'])) 
            return jsonify(r.json()) 
...
# Run
secret = "GmrXkjIdxggYf3tiI9vA5EZh9J8UmY5x"
